      ...
      <div className="container flex h-16 items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-6">
          ...
          <NavLink
            to="/auth/login"
            className={({ isActive }) =>
              cn("text-sm", isActive && "text-primary font-semibold")
            }
          >
            Login
          </NavLink>
          ...
          <div className="container py-2 grid gap-2">
            {nav.map((n) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  cn(
                    "block px-2 py-2 rounded hover:bg-secondary text-sm",
                    isActive && "text-primary font-semibold",
                  )
                }
              >
            ...
            <Link
              to="/auth/signup"
              onClick={() => setOpen(false)}
              className="block px-2 py-2 rounded bg-primary text-primary-foreground text-center text-sm font-medium"
            >
              Sign Up
            </Link>
          </div>
